...
#
def getOriginalValues(self, column):
    listOfValues = []
    rawdata = os.path.join(self.targetPathGOV + self.fileID)
...
class FoContour(object):

    def __init__(self, foData , time):
    ...
    def scalePCT(self):
        minFo = min(self.foData) - 10 # Baseline is set to n points under actual value
        maxFo = max(self.foData)
        rangeFo = maxFo - minFo
        foListScaled = [round((item - minFo) * (100.0/rangeFo),0) for item in self.foData]
        return foListScaled

    def scaleERB(self):
        foListScaled = [round((21.4 * math.log10((0.00437 * item) +1))*10,0) for item in self.foData]
        return foListScaled
...
def folderModeProcessing(targetFolder, chosenScaling):
    ...
    for fileName in listing:
        ...
        # fo and time scaling using class
